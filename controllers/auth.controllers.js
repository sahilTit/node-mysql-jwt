...
exports.register = async (req, res) => { 
    ...
    const users_jwt = await AuthService.createUser(userData);
    return res.json({
        data: users_jwt,
...
exports.login = async (req, res) => { 
    ...
    return res.status(400).json({ message: 'Unauthorized.', error : token });
}

exports.getUser = async (req, res) => {
    const users_jwt = await AuthService.findUserById(req.user.id);  
    return res.json({
        data: users_jwt,
        message: 'Success.'
    });
